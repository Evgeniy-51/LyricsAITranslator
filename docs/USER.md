...
Плагин **Lyrics AI Translator** для foobar2000 v2 (**32-bit или 64-bit** — пакет должен совпадать с foobar).
...
- Может запросить текст из lrclib.net и получить перевод через LLM (OpenAI-совместимый API)
...
4. Откройте ссылку на устройстве (та же Wi‑Fi сеть).
